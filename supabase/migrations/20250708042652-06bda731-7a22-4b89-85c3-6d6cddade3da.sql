-- Update the generate_weekly_snapshots function to use consistent tie-breaking
...
BEGIN
  ...
  -- Generate new snapshots with pool_id included and consistent tie-breaking
  WITH team_cumulative_stats AS (
    SELECT 
      pe.id as pool_entry_id,
      pe.pool_id,  -- Add pool_id here
      pe.participant_name,
      pe.team_name,
      pe.player_1,
      pe.player_2,
      ...
      pe.created_at, -- Include for tie-breaking
      ...
      COALESCE((
        ...
        AND (
          (bq.question_type = 'player_select' AND pe.bonus_answers->>bq.id::text = bq.correct_answer) OR
          (bq.question_type = 'dual_player_select' AND pe.bonus_answers->>bq.id::text = bq.correct_answer) OR
          (bq.question_type = 'yes_no' AND pe.bonus_answers->>bq.id::text = bq.correct_answer) OR
          (bq.question_type = 'number' AND pe.bonus_answers->>bq.id::text = bq.correct_answer)
        )
  ...
  ranked_teams AS (
    SELECT 
      *,
      ...
      -- Multi-level tie-breaking: total points DESC, weekly points DESC, bonus points DESC, created_at ASC
      ROW_NUMBER() OVER (
        ORDER BY 
          (cumulative_weekly_points + cumulative_bonus_points) DESC, 
          cumulative_weekly_points DESC, 
          cumulative_bonus_points DESC, 
          created_at ASC
      ) as current_rank
  ...
  SELECT 
    rt.pool_entry_id,
    rt.pool_id,  -- Include pool_id in SELECT
    week_num,
