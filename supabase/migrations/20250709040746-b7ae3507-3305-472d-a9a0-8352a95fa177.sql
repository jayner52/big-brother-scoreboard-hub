...
-- COMPREHENSIVE SEASON 27 CAST PRESERVATION MIGRATION
...
-- Create clean contestant groups (idempotent)
...
-- Step 4: Update Existing Pool Contestants
-- Force refresh existing pools to use the new clean Season 27 data
UPDATE contestants AS pool_contestants
SET 
  name = global_contestants.name,
  age = global_contestants.age,
  hometown = global_contestants.hometown,
  occupation = global_contestants.occupation,
  bio = global_contestants.bio,
...
AND pool_contestants.sort_order = global_contestants.sort_order;

-- Step 5: Verification - Log completion
