-- First, create a unique constraint on contestant_groups to support the ON CONFLICT
ALTER TABLE public.contestant_groups 
ADD CONSTRAINT unique_pool_group_name UNIQUE (pool_id, group_name);

-- Then insert the Season 27 contestants without the problematic ON CONFLICT
-- First, ensure default groups exist (only if they don't already exist)
DO $$
BEGIN
  -- Insert groups only if they don't exist
  IF NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group A') THEN
    INSERT INTO public.contestant_groups (pool_id, group_name, sort_order) VALUES (NULL, 'Group A', 1);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group B') THEN
    INSERT INTO public.contestant_groups (pool_id, group_name, sort_order) VALUES (NULL, 'Group B', 2);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group C') THEN
    INSERT INTO public.contestant_groups (pool_id, group_name, sort_order) VALUES (NULL, 'Group C', 3);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group D') THEN
    INSERT INTO public.contestant_groups (pool_id, group_name, sort_order) VALUES (NULL, 'Group D', 4);
  END IF;
END $$;
