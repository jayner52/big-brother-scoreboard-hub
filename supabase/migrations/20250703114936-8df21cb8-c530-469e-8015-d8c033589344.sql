-- First, let's update the generate_weekly_snapshots function to calculate cumulative points correctly
-- This function should only include points earned through the selected week
...
BEGIN
  ...
  -- Generate new snapshots with cumulative points calculation
  WITH team_cumulative_stats AS (
    SELECT 
      ...
      -- Calculate weekly points earned only up to the selected week
      COALESCE((
      ...
      -- Calculate bonus points earned only up to the selected week
      COALESCE((
      ...
      -- Calculate current week points for comparison
      COALESCE((
        SELECT SUM(we.points_awarded)
        FROM weekly_events we
        WHERE we.contestant_id IN (
  ...
  SELECT 
    ...
    rt.current_rank,
    COALESCE(rt.total_cumulative_points - pws.prev_total_points, rt.current_week_points) as points_change,
...
-- Clean up duplicate POV rules by keeping the first occurrence
WITH duplicate_rules AS (
  SELECT id, 
         ROW_NUMBER() OVER (PARTITION BY description ORDER BY created_at) as rn
  FROM public.detailed_scoring_rules 
  WHERE description = 'Power of Veto used to save this player'
)
...
-- Add real-time subscription for pool_entries table
ALTER TABLE public.pool_entries REPLICA IDENTITY FULL;