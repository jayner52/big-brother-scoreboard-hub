...
-- Update the trigger function to handle UUID event_types properly
CREATE OR REPLACE FUNCTION public.handle_special_event_status_changes()
RETURNS TRIGGER AS $$
DECLARE
  event_subcategory TEXT;
BEGIN
  -- Get the subcategory from the scoring rule
  ...
  IF event_subcategory IN ('self_evicted', 'removed_production') THEN
    ...
    WHERE contestant_id = NEW.contestant_id 
      ...
      AND event_type IN (
        SELECT id FROM public.detailed_scoring_rules 
...
-- Recreate the trigger without the WHEN clause since we're checking inside the function
