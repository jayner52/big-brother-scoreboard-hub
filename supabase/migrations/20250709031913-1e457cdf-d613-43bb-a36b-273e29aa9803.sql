-- CLEAR SEASON 27 DATA AND INSERT REAL BB27 CAST AS GLOBAL DEFAULTS
-- Keep existing global groups, only delete Season 27 contestants

-- Step 1: Delete ALL Season 27 contestants only
DELETE FROM contestants WHERE season_number = 27;

-- Step 2: Ensure we have the correct global groups (will skip if they exist)
INSERT INTO contestant_groups (pool_id, group_name, sort_order) 
SELECT NULL, 'Group A', 1
WHERE NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group A');

INSERT INTO contestant_groups (pool_id, group_name, sort_order) 
SELECT NULL, 'Group B', 2
WHERE NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group B');

INSERT INTO contestant_groups (pool_id, group_name, sort_order) 
SELECT NULL, 'Group C', 3
WHERE NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group C');

INSERT INTO contestant_groups (pool_id, group_name, sort_order) 
SELECT NULL, 'Group D', 4
WHERE NOT EXISTS (SELECT 1 FROM contestant_groups WHERE pool_id IS NULL AND group_name = 'Group D');
