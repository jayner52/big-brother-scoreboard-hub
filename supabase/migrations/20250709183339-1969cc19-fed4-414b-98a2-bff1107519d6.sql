-- Phase 2: Rebuild Eviction Status Logic - Core Detection Functions (Fixed UUID Cast)
...
BEGIN
  ...
  -- Then mark evicted contestants as inactive
  ...
  -- Update evicted contestants to inactive
  UPDATE public.contestants 
  SET is_active = false
  WHERE id IN (SELECT contestant_id FROM final_evicted)
  AND pool_id = target_pool_id;
