-- Phase 2: Rebuild Eviction Status Logic - Core Detection Functions
...
BEGIN
  -- First, get all evicted contestants for this pool
  WITH evicted_contestants AS (
    SELECT DISTINCT contestant_id 
    FROM public.get_evicted_contestants(target_pool_id)
  ),
  ...
  LEFT JOIN final_evicted fe ON c.id = fe.contestant_id
  WHERE c.pool_id = target_pool_id
  AND c.id = contestants.id;
