...
missing_events AS (
  SELECT 
    'in_showmance' as subcategory, 'In a Showmance' as description, '💕' as emoji, 2 as points, true as is_active
  UNION ALL SELECT 
...
SELECT 
  ...
  me.is_active
FROM pool_ids p
CROSS JOIN missing_events me
WHERE NOT EXISTS (
  SELECT 1 FROM public.detailed_scoring_rules existing
  WHERE existing.pool_id = p.pool_id 
    AND existing.category = 'special_events'
    AND existing.subcategory = me.subcategory
);
