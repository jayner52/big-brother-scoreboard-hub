-- Fix get_contestants_evicted_up_to_week to include special_events table
...
BEGIN
  ...
  -- Get evictions from special_events table up to target week
  SELECT 
    se.contestant_id,
    c.name as contestant_name,
    se.event_type as eviction_type,
  ...
  WHERE se.pool_id = target_pool_id
    ...
    AND se.event_type IN ('self_evicted', 'removed_production')
