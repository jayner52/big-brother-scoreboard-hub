-- Phase 2: Rebuild Eviction Status Logic - Core Detection Functions (Simplified)
...
RETURNS TABLE(evicted_contestant_id uuid, eviction_source text, eviction_week integer) AS $$
BEGIN
  RETURN QUERY
  WITH evicted_from_results AS (
    -- Get evictions from weekly_results (normal evictions)
    SELECT 
      c.id as evicted_contestant_id,
  ...
  evicted_from_events AS (
    ...
    SELECT 
      we.contestant_id as evicted_contestant_id,
...
BEGIN
  -- First, mark all contestants as active
  UPDATE public.contestants 
  SET is_active = true
  WHERE pool_id = target_pool_id;

  -- Mark evicted contestants as inactive
  UPDATE public.contestants 
  SET is_active = false
  WHERE id IN (
